# Huong dan su dung VIETCLAW

## Tong quan

VIETCLAW la kho du lieu phap luat Viet Nam duoc cau truc hoa cho he thong RAG (Retrieval-Augmented Generation). Tai lieu nay huong dan cach su dung du lieu va cac cong cu di kem.
...
```bash
# Clone repository
git clone https://github.com/trustydev212/vietclaw.git
cd vietclaw
...
collection = client.get_collection("vietclaw_laws")
...
collection = chroma.get_collection("vietclaw_laws")

def query_vietclaw(question: str) -> str:
...
# Su dung
answer = query_vietclaw("Quyen loi cua nguoi lao dong khi bi sa thai trai phap luat?")
print(answer)
```
